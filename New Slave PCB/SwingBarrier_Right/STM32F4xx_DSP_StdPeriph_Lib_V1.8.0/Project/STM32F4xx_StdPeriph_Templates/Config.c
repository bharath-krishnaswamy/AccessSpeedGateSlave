...
//      enc_pos = 125;
  ...
  if((is_gate_moving == 1) || (f == 1))
  {
    ...
    if(f == 1)
      enc_pos = (int)(temp - ((temp + 180)/ m));//-180 is the position we need
    else 
      enc_pos = (int)(temp - (temp / m));
  }
  else if((is_gate_moving == 0) && (f == 0))
    enc_pos = (int)(180 - (180 / m));//temp gives current psoition of the gate, required only during emergency stopping or fire mode 
  TIM_OCInitTypeDef TIM_OCStruct; 
  TIM_OCStruct.TIM_OCMode = TIM_OCMode_PWM2;
  TIM_OCStruct.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCStruct.TIM_OCPolarity = TIM_OCPolarity_Low;
