...
void Init_Sequence(void)
{
  ...
  if ((temp != 0) || (temp != 1) || (temp != -1))
  {
    if (( temp < -2 ) && ( temp >= -10 ))
    {
      GPIO_WriteBit(GPIOE, GPIO_Pin_8, Bit_RESET);//go anticlockwise
      position = temp;
...
//      }
    ...
    {
      GPIO_WriteBit(GPIOE, GPIO_Pin_8, Bit_RESET);//go clockwise
      position = temp + 5;
      counter = temp + 5;//change on 19th march...
      GPIO_WriteBit(GPIOE, GPIO_Pin_9, Bit_SET); //brake set
      TIM_Cmd(TIM4, ENABLE);
//      if((position == 0))
//      {
...
//      }
     ...
     {
       GPIO_WriteBit(GPIOE, GPIO_Pin_8, Bit_SET);//go anticlockwise
       position = temp - 5;
       counter = temp -5;//change on 19th march...
       GPIO_WriteBit(GPIOE, GPIO_Pin_9, Bit_SET); //brake set
       TIM_Cmd(TIM4, ENABLE);
