...
void TM_PWM_Init(void) {
    ...
    TIM_OCStruct.TIM_Pulse = 849; /*  10% duty cycle */
...
/**
  * @brief  Configures the PWM on time for modifying speed if the gate is moving
  * @param  speed value from master via uart from 1 to 9
  * @retval None
  */
void Emergency_PWM(uint8_t s)
{
  ...
  TIM_OCStruct.TIM_OutputState = TIM_OutputState_Enable;
  TIM_OCStruct.TIM_OCPolarity = TIM_OCPolarity_Low;
  TIM_OCStruct.TIM_Pulse = 419 + (s * 420);//10% to 50% PWM pulse width
  TIM_OC1Init(TIM4, &TIM_OCStruct);
  TIM_OC1PreloadConfig(TIM4, TIM_OCPreload_Enable);
}
